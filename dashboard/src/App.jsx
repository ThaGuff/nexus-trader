    ...
    <div style={{position:'fixed',inset:0,background:'#000000cc',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:C.card2,border:`1px solid ${C.border2}`,borderRadius:14,padding:28,width:'100%',maxWidth:460,maxHeight:'90vh',overflowY:'auto'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:22}}>
          <span style={{color:C.text,fontSize:17,fontWeight:800}}>Bot Settings</span>
          <button onClick={onClose} style={{color:C.muted,background:'none',border:'none',cursor:'pointer',fontSize:22,lineHeight:1}}>×</button>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
          {[['Max Trade ($)','maxTradeUSD',5,10000],['Stop Loss (%)','stopLossPct',0.5,50],['Take Profit (%)','takeProfitPct',1,100],['Max Drawdown (%)','maxDrawdownPct',5,50]].map(([l,k,mn,mx])=>(
            <div key={k}>
              <div style={{color:C.muted,fontSize:10,fontWeight:600,marginBottom:6,textTransform:'uppercase',letterSpacing:'0.08em'}}>{l}</div>
              <input type="number" min={mn} max={mx} step="0.5" value={form[k]} onChange={e=>set(k,e.target.value)} style={inp}/>
            </div>
          ))}
        </div>
        ...
        <div style={{background:'#f59e0b0a',border:'1px solid #f59e0b20',borderRadius:8,padding:'10px 12px',marginBottom:20}}>
          <p style={{color:'#d97706',fontSize:11,lineHeight:1.6}}>⚠ Settings save immediately and persist across restarts.</p>
        </div>
